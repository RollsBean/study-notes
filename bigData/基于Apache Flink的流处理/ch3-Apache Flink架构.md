...
键值分区状态会按照键值进行维护或访问。 Flink 为每个键值都维护了一个状态实例。

键值分区状态原语：

**单值状态**（value state）  
&nbsp; &nbsp; &nbsp; &nbsp;每个键对应存储一个任意类型的值。该值也可以是复杂数据结构。

**列表状态**（list state）  
&nbsp; &nbsp; &nbsp; &nbsp;每个键对应一个值的列表。列表中的条目可以是任意类型。

**映射状态**（map state）  
&nbsp; &nbsp; &nbsp; &nbsp;每个键对应一个键值映射，该映射（map）的键和值可以是任意类型。

### 状态后端

有状态的算子任务一般需要在每一条记录到来时读写状态，所以高效访问状态对低延迟很重要。为了高效访问状态，每个并行的任务都会把状态存在本地。具体的存储、
访问和维护由**状态后端**决定。状态后端负责**本地存储管理**和**将状态以检查点的方式写入远程存储**。

Flink提供的一类状态后端会把状态存在 JVM 堆中；另一类会把对象序列化到 RocksDB 中。前者访问快，后者可存储的数量大。

由于 Flink 是分布式的，而状态只在本地维护，所以状态后端需要把任务状态以检查点的方式写入到远程存储，远程存储可以是分布式文件系统，也可以是某个数据库
系统。不同的状态后端实现不一样，比如 RocksDB 支持**增量检查点**。

### 有状态算子的扩缩容

Flink 对不同的状态类型提供了四种扩缩容模式。

**带有键值分区状态的算子**在扩缩容时会根据新的任务数量对键值重新分区。为了降低状态迁移成本，Flink 会把所有的键值分为不同的的**键值组**（key group），
FLink 以此为单位为键值重新分配任务。

**带有算子列表状态的算子**会对列表中的条目进行重新分配。理论上会重新平均分配列表。

**带有算子联合列表状态的算子**会把列表的全部条目广播到全部任务上。

**带有算子广播状态的算子**会把状态拷贝到全部新任务上。这样能保证所有任务的状态相同。

![flink broadcast state](../../image/bigData/基于Apache%20Flink的流处理/flink%20broadcast%20state.png)

## 检查点、保存点及状态恢复

Flink 需要处理进程被强制关闭、机器故障以及网络连接中断。

### 一致性检查点

Flink 的故障恢复机制需要基于应用状态的一致性检查点。通过朴素算法建立一致性检查点。步骤为：

1. 暂停接受所有输入流。
2. 等待系统中的数据处理完。
3. 将所有任务状态拷贝到远程吃就好存储，生成检查点。
4. 恢复所有数据流的接收。

Flink 采用 Chandy-Lamport 算法。
...
如果算子都将状态写入检查点并从中恢复，并且所有输入流的消费位置都能重置到检查点生成那一刻，那么该检查点和恢复机制就能为整个应用的状态提供精确一次的
一致性保障。

### Flink 检查点算法

基于朴素算法的故障恢复需要停止一切任务，这样对于有中等延迟要求的应用不切实际。FLink 基于 Chandy-Lamport 分布式快照算法来实现的。该算法会把生成
检查点的过程和处理过程分离。

Flink 的检查点算法中会用到**检查点屏障**（checkpoint barrier）。为了标识检查点，每个检查点屏障都会带有一个检查点编号。两个数据源流入的情况下，
对于已经提供分隔符的分区，他们新到来的记录会被缓冲起来，不能处理。这个等待的过程被称为**分隔符对齐**。

### 检查点对性能的影响

状态后端支持异步生成检查点，生成检查点时，状态后端会为当前状态创建一个本地拷贝，本地拷贝完成后，任务就可以继续它的常规处理。
后台异步线程将本地状态快照拷贝到远程存储。
...
* 从保存点启动一个不同但相互兼容的应用。比如修复bug后重启应用。
* 用不同的并行度启动原应用，实现扩缩容。
* 在另一个集群上启动相同的应用。
* 利用保存点暂停某个应用，给更高优先级的应用腾出集群资源。
* 为保存点设置不同版本并将应用状态归档。
